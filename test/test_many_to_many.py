...
def test_many_to_many(app):
    ...
    test_user1=db.session.query(user.User).filter_by(email="test_email@email.com").one()
    login_user(test_user1)
    scrape(soup=soup, asin="B076V9P58R")

    test_user2=db.session.query(user.User).filter_by(email="test2_email@email.com").one()
    ...
    test_user1=db.session.query(user.User).filter_by(email="test_email@email.com").one()
    assert item.Item.query.join(
        users_items_association, (users_items_association.c.user_id == test_user1.id)
    ).filter(users_items_association.c.item_id == item.Item.id).one()
    # Test that item is attached to user 2
    test_user2=db.session.query(user.User).filter_by(email="test2_email@email.com").one()
    assert item.Item.query.join(
        users_items_association, (users_items_association.c.user_id == test_user2.id)
    ).filter(users_items_association.c.item_id == item.Item.id).one()
